...
mesh = Mesh("obj_models/clubbing_dress.obj", scale=0.4, rot=ti.math.vec3(90.0, 0.0, 0.0),trans=ti.math.vec3(0.3, 0.2, 0.3))
# mesh = Mesh("obj_models/cube.obj", scale=0.1, rot=ti.math.vec3(90.0, 0.0, 0.0),trans=ti.math.vec3(0.1, 0., 0.0))
...
while window.running:
    ...
    # scene.particles(mesh.mesh.verts.x, radius = sim.radius, color=(0, 1, 0))
    scene.mesh(mesh.mesh.verts.x, mesh.face_indices, color=(0.5, 0.5, 0.5))
    # scene.mesh(static_mesh.mesh.verts.x, static_mesh.face_indices, color=(0.5, 0.5, 0.5))
    scene.lines(mesh.mesh.verts.x, width=0.5, indices=mesh.edge_indices, color=(0., 0., 0.))
    canvas.scene(scene)
    window.show()
