...
def substep_PD(k: float):
    ...
    for part in range(1):
        ...
        offset = 0
        c_tilde[offset] = c[offset] / b[offset]
        # ti.loop_config(serialize=True)
        for id in range(1, size - 1):  # lb+1 ~ ub-1
            i = id + offset
            # tmp = ti.math.inverse(b[i] - a[i] * c_tilde[i - 1])
            c_tilde[i] = c[i] / (b[i] - a[i] * c_tilde[i - 1])
        #
        d_tilde[offset] = d[offset] / b[offset]
        # ti.loop_config(serialize=True)
        ...
        for i in range(0, size - 1):
            ...
            dx[idx] = d_tilde[idx] - dx[idx + 1] * c_tilde[idx]
...
@ti.kernel
def substep_Newton(k: float, fix: bool):
...
def run_pbf():
    ...
    if solver_type == 0:
        ...
        for _ in range(pbf_num_iters):
            k = PR * time_delta ** 2
            substep_Newton(k, definiteness_fix)
...
def main():
    ...
    def show_options():
        ...
        with gui.sub_window("XPBD Settings", 0., 0., 0.3, 0.7) as w:
            ...
            definiteness_fix = w.checkbox("definiteness fix", definiteness_fix)
