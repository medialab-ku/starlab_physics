...
max_num_iter = 1000
err = ti.field(dtype=float, shape=max_num_iter)
...
@ti.kernel
def substep_Euler_GC(Px: ti.template(), ci: int, x: ti.template()):
...
def forward():
    ...
    e_0 = 0.0
    e_0 += compute_e_momentum(x_k)
    e_0 += compute_e_attachment(x_k, k_col)
    e_0 += compute_e_spring(x_k, k)
    # print(e_0)
    err[itr_cnt] = e_0
    if solver_type < 3:
        for _ in range(num_iters):
            ...
            if solver_type == 0:
                ...
                ci = itr_cnt % num_colors
                # print(ci)
                P_grad.fill(0.0)
                substep_Euler_GC(P_grad, ci, grad)
            ...
            e += compute_e_attachment(x_k, k_col)
            e += compute_e_spring(x_k, k)
            err[itr_cnt] = e
    ...
    # print(e)
        ...
        for _ in range(num_iters):
            ...
            for ci in range(num_colors):
                ...
                # for i in range(num_edges):
                # substep_GS(x_k,  k)
                substep_Euler_GS(P_grad, ci, grad)
    ...
    if print_stat:
        for i in range(itr_cnt - 1):
            print(abs(err[i] - err[itr_cnt - 1]) / err[itr_cnt - 1])
