...
class Solver:
    def __init__(
        ...
        self.dx_c_dup = self.mesh_dy.dx_c_dup
    ...
    def solve_spring_constraints_euler_path_tridiagonal_x(self,
        ...
        self.mesh_dy.dx.fill(0.0)
        self.mesh_dy.dx_c_dup.fill(0.0)
        ...
        #     c += 0.5 * x01.dot(x01)
        #     self.dx_c_dup[v0_d] -= x01
        #     self.dx_c_dup[v1_d] += x01
        ...
        #     # https://en.wikipedia.org/wiki/Tridiagonal_matrix_algorithm
            ...
            self.dx_dup[offset + size - 1] = self.d_tilde[offset + size - 1]
            ti.loop_config(serialize=True)
            for i in range(0, size - 1):
                idx = size - 2 - i + offset # ub-1 ~ lb

                self.dx_dup[idx] = self.d_tilde[idx] - self.c_tilde[idx] * self.dx_dup[idx + 1]
        ...
        #
        for i in self.mesh_dy.x:
            self.mesh_dy.y[i] += (self.mesh_dy.dx[i] / self.mesh_dy.num_dup[i])
        # schur = 0.0
        # for i in self.dx_c_dup:
        #     dx_c_dup_i = self.dx_c_dup[i]
        #     schur += (dx_c_dup_i.dot(dx_c_dup_i) / self.b[i])
        #
        # nabla_c_T_dx = 0.0
        # for i in self.dx_c_dup:
        #     dx_c_dup_i = self.dx_c_dup[i]
        #     dx_dup_i = self.dx_dup[i]
        #     nabla_c_T_dx += dx_dup_i.dot(dx_c_dup_i)

        ld = 0.0
        # if c > 1e-3:
        #     ld = (c - nabla_c_T_dx) / schur
        #
        # for i in self.mesh_dy.y_dup:
        #     self.mesh_dy.y_dup[i] += self.dx_dup[i]
